...
export class SavedLocationsService {
  ...
  async save(userId: string, dto: SaveLocationDto) {
    try {
      return await this.savedRepo.save(
  ...
  async listCollections(userId: string) {
    const rows = await this.savedRepo
      ...
      .getRawMany();
    return rows.map((r) => r.collectionName as string);
  }

  async findByUserAndLocation(userId: string, locationId: string) {
    return this.savedRepo.findOne({ where: { userId, locationId } });
  }
}
