...
export async function openGoogleMapsTo(dest: Coords, origin?: Coords | null) {
  ...
  const nativeUrl = origin
    ? Platform.select({
        ios: `comgooglemaps://?saddr=${origin.lat},${origin.lng}&daddr=${destStr}&directionsmode=driving`,
        android: `google.navigation:q=${destStr}`,
        default: webUrl,
      })
    : Platform.select({
        ios: `comgooglemaps://?daddr=${destStr}&directionsmode=driving`,
        android: `google.navigation:q=${destStr}`,
        default: webUrl,
      });
