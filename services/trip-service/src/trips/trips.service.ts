...
export class TripsService {
  ...
  async patch(userId: string, tripId: string, dto: PatchTripDto, expectedVersion?: number): Promise<TripDetailDto> {
    ...
    if (modeChanged) {
      const days = await this.daysRepo.find({ where: { tripId } });
      for (const day of days) {
        const dayStops = (await this.stopsRepo.find({ where: { tripId, tripDayId: day.id }, order: { position: 'ASC' } }));
        await this.travelLegs.recomputeDayLegs(dayStops, trip.defaultTravelMode);
        if (dayStops.length) await this.stopsRepo.save(dayStops);
      }
    }
  ...
  private assertVersion(trip: TripEntity, expectedVersion?: number) {
    ...
    if (trip.version !== expectedVersion) {
      throw new ConflictException({ error: 'version_conflict', currentVersion: trip.version });
    }
  }
  ...
  private toDetail(
    ...
    const dayDtos: TripDayDto[] = days.map((day) => {
      ...
      const scheduleResult = computeDaySchedule({
        ...
        dayStartTime,
        timezoneOffset: trip.timezone ?? '+07:00',
